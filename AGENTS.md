...
当我提出想法的时候，不需要迎合我；我需要向你学习，务必客观公正。请直接指出问题、成本、风险和更优方案，而不是顺着我的话说"可以 / 好方向"。肯定的地方要说明依据，否定的地方要给出替代建议；如果我漏掉了关键约束，请先补全约束再给结论。
...
## 协作模式约定（2026-09-12 用户拍板）

1. **默认单兵模式**：主理人直接实现，不建 SOP 团队。单人项目上下文已沉淀在
   `.workbuddy/memory/` 与 `docs/architecture/` 中，四人团队的编排成本高于收益。
2. **仅冷启动开团队**：全新模块（零上下文、跨多端的大功能）才走 PRD → 架构 → 实现 → QA 完整 SOP。
3. **文档生命周期**：`docs/` 只放活跃文档；已完成任务与过期方案移入 `docs/archive/YYYY-MM/`，
   用 `git mv` 保留历史，不删除。ADR 永久保留在 `docs/adr/`。
4. 实证依据：2026-09-11 精品阅读站改造中，架构代理两次设计错误被收回设计权、工程代理
   spawn 两次失败，主理人亲写 T04 当日完成（259 测试全绿、黄金回放 8/8 达标）。
